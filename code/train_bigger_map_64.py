...
from core.utils.utils import import_mapf_instance, inverse_sigmoid, get_scene_19, dense_input_data, get_scene_80, visualize_heatmap
...
def main():
    ...
    train_scene_list = get_scene_80("test_70", "bigger_heatmap", dataset="train")
    valid_scene_list = get_scene_80("test_70", "bigger_heatmap", dataset="valid")
    ...
    if args.model == None:
        ...
        if args.model == "unet_64":
            model = UNet_2D_64(2, 1, 102).to(device)
            model_name = "unet_64"

        elif args.model == "cnn_mlp_64":
            model = cnn_mlp_64(102, 4096).to(device)
            model_name = 'cnn_mlp_64'
    ...
    if os.path.exists(save_path):
        ...
        if len(saved_model_list) == 0:
            ...
            # print(np.argmin(valid_loss_list))
            # return

            valid_loss_epoch = np.argmin(valid_loss_list)
            valid_f.close()
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        print("Epoch: {} / {}\nTrain loss: {}".format(epoch, EPOCHS, avg_train_loss))
...
def train(train_scene_list, BATCH_SIZE, model, loss_f, optm):
    ...
    for train_scene in train_scene_list:
        ...
        obs_map = torch.reshape(torch.Tensor(obs_map), sorted_heatmap_array.shape).to(device)
        h_val = torch.reshape(torch.Tensor(h_val), sorted_heatmap_array.shape).to(device)
...
def valid(valid_scene_list, BATCH_SIZE, model, loss_f):
    ...
    with torch.no_grad():
        for valid_scene in valid_scene_list:
            ...
            obs_map = torch.reshape(torch.Tensor(obs_map[:BATCH_SIZE]), sorted_heatmap_array[:BATCH_SIZE].shape).to(device)
            h_val = torch.reshape(torch.Tensor(h_val[:BATCH_SIZE]), sorted_heatmap_array[:BATCH_SIZE].shape).to(device)
