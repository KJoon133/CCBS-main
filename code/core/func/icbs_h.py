...
from core.func.a_star import a_star, get_sum_of_cost, compute_heuristics, get_location
...
from core.func.epea_star_class import EPEA_Star
...
def generated_collisiin_graph(paths):
    edges = []
    for i in range(len(paths) - 1):
        for j in range(i + 1, len(paths)):
            if count_all_collisions_pair(paths[i], paths[j]) > 0:

                edges.append((i, j))
    return edges

def vertex_cover(paths):
    vertex_cover = []
    edges = generated_collisiin_graph(paths)
    ...
    while len(edges):
        ...
        count_item = Counter(c)
        most_vertex = count_item.most_common(n=1)[0][0]
        vertex_cover.append(most_vertex)
        remove_list = []

        if count_item.most_common(n=1)[0][1] == 1:
            for v in edges:
                vertex_cover.append(v[0])
            break

        for j in range(len(edges)):
            if most_vertex == edges[j][0] or most_vertex == edges[j][1]:
                remove_list.append(edges[j])

        remove_set = set(remove_list)

        edges = [k for k in edges if k not in remove_set]

    h = len(set(vertex_cover))
...
class ICBS_h_Solver(object):
    ...
    def find_solution(self, disjoint, a_star_version):
        ...
        root['h'] = vertex_cover(root['paths'])
        self.push_node(root)
        ...
        while len(self.open_list) > 0:
                ...
                if paths is not None:
                    ...
                    if constraint['positive']:
                        ...
                        for v_ma in violating_ma_list:
                            ...
                            astar_v_ma = AStar(self.my_map, self.starts, self.goals, self.heuristics, v_ma_list,
                                               q['constraints'])
                    ...
                    if collision_type == 'cardinal':
                        assert bypass_found(p['cost'] - p['h'], q['cost'], len(p['ma_collisions']),
                                            len(q['ma_collisions'])) == False

                        h_value = vertex_cover(q['paths'])
                    ...
                    if collision_type != 'cardinal' \
                            and bypass_found(p['cost']-p['h'], q['cost'], len(p['ma_collisions']), len(q['ma_collisions'])):
                        print('> Take Bypass')
                        self.push_node(q)
            ...
            if should_merge(collision, p, 7):
                ...
                ma_astar = AStar(self.my_map, self.starts, self.goals, self.heuristics, list(meta_agent),
                                 updated_constraints)
                ...
                if ma_paths:
                    ...
                    updated_node = generate_child(updated_constraints, updated_paths, p['agent_collisions'],
                                                  updated_ma_list)
