...
def compute_heuristics_Euclidean(my_map, goal):
    open_list = []
    closed_list = dict()
...
def compute_heuristics_costmap(my_map, goal, costmap):
    # Use Dijkstra to build a shortest-path tree rooted at the goal location
    costmap = costmap[0]
    open_list = []
    closed_list = dict()
    ...
    h_values = dict()
    for loc, node in closed_list.items():
        h_values[loc] = node['cost'] + costmap[loc[0]][loc[1]]

    return h_values

def compute_heuristics_uniform_costmap(my_map, goal, costmap):
    # Use Dijkstra to build a shortest-path tree rooted at the goal location
    ...
    root = {'loc': goal, 'cost': 0}
    heapq.heappush(open_list, (root['cost'], goal, root))
    # print("heapq.heappush(open_list, (root['cost'], goal, root)):", open_list)
    closed_list[goal] = root
    ...
    h_values = dict()
    for loc, node in closed_list.items():
        h_values[loc] = node['cost'] + int(5*costmap[loc[0]][loc[1]])

    # print("goal:", goal , "\nh_values:", h_values, "\nh_values length:", len(h_values))
    # input("  ")
    # print("costmap:\n", costmap)
