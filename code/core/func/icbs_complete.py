...
class ICBSSolver_Cost_Data_generate(object):
...
class ICBS_Solver_Compare(object):
    """The high-level search of CBS."""
    ...
    def push_node_close(self, node, flag):
        if flag == "gen":
            heapq.heappush(self.gen_node_list, (self.num_of_generated, node['cost'], len(node['ma_collisions']), node))
        elif flag == "exp":
            heapq.heappush(self.exp_node_list, (self.num_of_expanded, node['cost'], len(node['ma_collisions']), node))
    ...
    def find_solution(self, disjoint, a_star_version):
        ...
        while len(self.open_list) > 0:
            ...
            if p['ma_collisions'] == []:
                ...
                return p['paths'], self.num_of_generated, self.num_of_expanded, total_CT # number of nodes generated/expanded for comparing implementations
